...
**mise-seq** bootstraps a **global, per-user CLI toolchain** on top of a **system-installed `mise`**.

It installs tools **sequentially** using `mise use -g TOOL@VERSION` and can run lifecycle hooks:
...
`mise install` may install multiple tools in parallel. In practice, parallel installs can fail when a tool depends on another backend/runtime that is not fully ready yet.

**mise-seq** makes the install order explicit (`tools_order`) and installs tools one-by-one via `mise use -g ...`.
...
2) `cue vet -c=false schema/mise-seq.cue tools.yaml -d '#MiseSeqConfig'` (schema validation)
...
A separate workflow (workflow_dispatch) can run a **pre-release sample install test** using the repo-provided `.tools/tools.yaml` in an isolated mise data directory.
